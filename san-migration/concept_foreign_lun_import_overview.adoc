...
儘管ONTAP不再需要專業服務人員進行遷移，但NetApp強烈建議專業服務人員參與除最簡單的遷移之外的所有遷移的範圍界定、規劃和培訓。

FLI的開發目的是將SAN LUN移轉至ONTAPFLI支援多種移轉需求、包括但不限於下列項目：

* 將來自 EMC、日立、惠普和其他供應商的異質儲存陣列之間的資料遷移到NetApp。
...
FLI 要求每個控制器上至少有一個實體 FC 端口，以便 LUN 在啟動器模式下直接遷移。建議使用兩個端口，每個光纖通道陣列一個，但也可以只使用一個端口。這些連接埠用於連接到來源陣列，需要進行分割和屏蔽，以便能夠查看和掛載來源 LUN。如果需要將連接埠從目標變更為啟動器，請參閱link:https://docs.netapp.com/us-en/ontap/san-admin/configure-fc-adapters-task.html["配置 FC 適配器"^] 。
